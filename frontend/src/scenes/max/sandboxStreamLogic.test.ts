...
describe('sandboxStreamLogic', () => {
    ...
    describe('telemetry parity (§ 10)', () => {
        it('emits TASK_RUN_STARTED once on the first _posthog/run_started frame', async () => {
        ...
        it('defaults cold_start to true when the run_started frame omits it', async () => {
            const captureSpy = jest.spyOn(posthog, 'capture').mockImplementation(() => undefined as any)
            logic.actions.openSseForRun({ taskId: 'task-1', runId: 'run-1' })
            ...
            const call = captureSpy.mock.calls.find((c) => c[0] === 'task_run_started')
            expect(call?.[1]).toEqual(expect.objectContaining({ cold_start: true }))
        })
