...
export const maxThreadLogic = kea<maxThreadLogicType>([
    ...
    listeners(({ actions, values, cache, props }) => ({
        ...
        setQuestion: ({ question }) => {
            // Sandbox pre-warming (05_SANDBOX § 8). Debounce on the first non-whitespace keystroke
            // so the sandbox boots while the user is still typing; release if the input empties.
        ...
        askMax: async ({ prompt, addToThread = true, uiContext }) => {
            ...
            // A sent message consumes any sandbox pre-warm (05_SANDBOX § 8): the warm Run is the
            // in-progress run the sandbox routing follows up on, so cancel pending timers and clear
            // the flag WITHOUT issuing a release/DELETE.
            cache.disposables.dispose('prewarm-debounce')
            cache.disposables.dispose('prewarm-release')
            cache.prewarmed = false
    ...
    selectors({
        ...
        filteredCommands: [
            ...
            (
                ...
                // Sandbox runtime drops core-memory commands (02_CORE § 8); LangGraph keeps the full set.
