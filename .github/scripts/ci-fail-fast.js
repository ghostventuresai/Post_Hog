...
module.exports = async ({ github, context, core }) => {
    ...
    const verb = live ? 'Cancelling' : '[dry-run] Would cancel'
    ...
    for (const r of cancellable) {
        core.notice(`${verb}: ${r.name} #${r.id} (${r.status})`)
        if (live) {
    ...
    const rows = cancellable.map((r) => `| ${r.name} | ${r.status} |`).join('\n')
    await core.summary
        .addHeading(`CI Fail-Fast — ${mode.toUpperCase()}`)
        .addRaw(
            `Triggered by **${source.name}** failing on \`${shortSha(headSha)}\`${prNumber ? ` (PR #${prNumber})` : ''}.\n\n`
        )
        .addRaw(
            cancellable.length
                ? `${live ? 'Cancelled' : 'Would cancel'} **${cancellable.length}** sibling run(s):\n\n| Workflow | Status |\n| --- | --- |\n${rows}\n`
    ...
    core.setOutput(
        'summary',
        cancellable.length
            ? `${verb} ${cancellable.length} sibling CI run(s) on ${shortSha(headSha)}${prNumber ? ` (PR #${prNumber})` : ''} after ${source.name} failed: ${cancellable.map((r) => r.name).join(', ')}`
            : ''
    )
}
