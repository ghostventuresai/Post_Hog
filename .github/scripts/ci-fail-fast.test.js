...
function makeMocks({ runs, mode }) {
    ...
    const summaryChain = {}
    for (const m of ['addHeading', 'addRaw']) {
        summaryChain[m] = () => summaryChain
    }
    summaryChain.write = recordingFn(async () => {})
    const core = {
        notice: recordingFn(),
        warning: recordingFn(),
...
describe('ci-fail-fast', () => {
    ...
    describe('run()', () => {
        ...
        it('dry mode cancels nothing but reports the count', async () => {
            const { github, core, context, outputs, cancelWorkflowRun } = makeMocks({ runs: siblings, mode: 'dry' })
        ...
        it('on mode cancels each sibling run', async () => {
            const { github, core, context, outputs, cancelWorkflowRun } = makeMocks({ runs: siblings, mode: 'on' })
            await failFast({ github, context, core })
        ...
        it('emits an empty summary output when nothing is in flight', async () => {
            const { github, core, context, outputs } = makeMocks({ runs: [siblings[2]], mode: 'on' })
            await failFast({ github, context, core })
