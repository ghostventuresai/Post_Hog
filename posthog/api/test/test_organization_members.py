...
class TestOrganizationMembersForAccountAPI(APIBaseTest):
    ...
    def test_returns_members_of_target_org_when_flag_enabled(self, _mock_flag):
        org = Organization.objects.create(name="Customer Org")
        first = User.objects.create(email="cust1@example.com", first_name="Ada", distinct_id="distinct-1")
        first.join(organization=org)
        second = User.objects.create(email="cust2@example.com", first_name="Grace", distinct_id="distinct-2")
        second.join(organization=org)

        response = self.client.get(self._url(org.id))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        results = response.json()["results"]
