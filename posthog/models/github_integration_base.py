...
class GitHubIntegrationBase:
    ...
    def _gh_graphql(self, query: str, variables: dict[str, Any], *, endpoint: str, timeout: int = 10) -> dict:
        ...
        for attempt in range(2):
            ...
            if errors:
                # GitHub returns partial `data` alongside per-field `errors` when
                # the installation lacks a single field's permission (e.g.
                # statusCheckRollup needs "Checks: read"). Keep whatever data came
                # back — only fail when nothing usable was returned.
