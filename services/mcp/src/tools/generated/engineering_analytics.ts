...
const pullRequests = (): ToolBase<typeof PullRequestsSchema, WithPostHogUrl<Schemas.PullRequestListItem[]>> => ({
    name: 'pull-requests',
    schema: PullRequestsSchema,
    handler: async (context: Context, params: z.infer<typeof PullRequestsSchema>) => {
        const projectId = await context.stateManager.getProjectId()
        const result = await context.api.request<Schemas.PullRequestListItem[]>({
            method: 'GET',
