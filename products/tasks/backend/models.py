...
class CodeWorkflowConfig(TeamScopedRootMixin):
    """A user's PostHog Code "Home" workflow: situation → quick-action bindings.

    One row per (team, user). `version` is monotonic — only `save` bumps it — and
    powers optimistic concurrency on the save endpoint. `bindings` stores the
    WorkflowBindings wire shape.
    """
...
class CodePrSnapshot(TeamScopedRootMixin):
    """Polled GitHub pull-request metadata, shared across a team's workstreams.

    One row per (team, pr_url), so the same PR is polled once per cycle regardless
    of how many users track it. Per-user flags (is-author / is-requested-reviewer)
    are NOT stored here — they're resolved when the worker builds each user's
    `CodeWorkstream`. `fingerprint` drives cheap change detection between cycles.
    """
...
class CodeWorkstream(TeamScopedRootMixin):
    """A grouped, classified unit of code work for one user's Home view.

    One row per (team, user, key) where `key` is the grouping key
    (PR URL → repo+branch → worktree path). Rebuilt every cycle by the
    `evaluate-team-code-workstreams` worker. Active-agent rows are NOT persisted
    here — the API computes the live strip from TaskRun.
    """
