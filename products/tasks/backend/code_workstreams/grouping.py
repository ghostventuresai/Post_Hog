...
@dataclass
class TaskInput:
    """A task the worker considers for grouping. Built from Task + latest TaskRun."""
...
@dataclass
class PrInput:
    """Resolved PR snapshot for a task."""
...
def _is_actively_running(task: TaskInput, now: int, has_pr: bool) -> bool:
    ...
    # agent moved past working into review; a long-idle run is a status the
    # server never closed out — both fall through to grouping/classify().
...
def workstream_key(task: TaskInput, pr_url: Optional[str]) -> Optional[str]:
    """Grouping key precedence: resolved PR URL → repo+branch → worktree path.
    Tasks with none are skipped, so Home stays a view of actual code work."""
    if pr_url:
        return f"pr:{pr_url}"
    repo = task.repo_name
    branch = task.branch
...
def build_workstreams(
    ...
    """Group tasks into workstreams and classify their state.

    Tasks whose run is a currently-live agent are skipped — the API surfaces
    those as the live "active agents" strip, computed fresh from TaskRun at read
    time.
    """
    ...
    for key, group_tasks in groups.items():
        ...
        # First task that resolves to a PR carries the snapshot; PR-grouped
        # tasks share the URL anyway.
