...
def _pr_wire(pr: PrInput) -> dict:
    """Per-user-resolved PrSnapshot wire shape (camelCase) for the API."""
    return {
        "url": pr.url,
        "number": pr.number,
        "title": pr.title,
...
def _build_pr_input(snapshot: CodePrSnapshot) -> PrInput:
    # Workstreams group a user's own tasks, so a task-derived PR is authored by
    # that user. Requested-reviewer resolution isn't implemented yet, so default
    # it to False.
    return PrInput(
        url=snapshot.pr_url,
...
def rebuild_team_workstreams(input: RebuildTeamWorkstreamsInput) -> RebuildTeamWorkstreamsOutput:
    """Group each user's recent tasks into workstreams, classify, and persist.

    Reads the PR snapshots polled earlier this cycle, runs the pure grouping +
    classification, upserts CodeWorkstream rows per user, and prunes rows whose
    grouping key no longer exists."""
    now = timezone.now()
        ...
        for user_id, user_tasks in by_user.items():
            ...
            for task in user_tasks:
                ...
                if pr_url and pr_url in snapshots_by_url:
                    ...
                    pr_by_task[task_input.id] = _build_pr_input(snapshot)
