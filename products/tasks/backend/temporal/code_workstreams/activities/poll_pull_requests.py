...
def poll_pull_requests_for_team(
    ...
    *,
    ...
    """Poll each PR's GitHub state once and upsert a shared CodePrSnapshot.

    Sequential and heartbeated; one bad PR is logged and skipped; a secondary
    rate limit stops the cycle (the next cycle resumes). Writes are skipped when
    a PR's fingerprint is unchanged. The activity wraps this with the Temporal
    heartbeat; callers outside an activity (e.g. the management command) pass
    `heartbeat=None`."""
