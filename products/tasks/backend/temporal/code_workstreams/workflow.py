...
class EvaluateTeamCodeWorkstreamsWorkflow(PostHogWorkflow):
    """Per-team pipeline: resolve tracked PRs → poll their GitHub state → rebuild
    every user's classified workstreams. Each team runs as its own workflow so
    history stays bounded and one team's failure is isolated."""
...
class EvaluateCodeWorkstreamsWorkflow(PostHogWorkflow):
    """Scheduled dispatcher: fan out a per-team evaluation for every team with
    recent code activity, bounded so we don't overwhelm the worker or GitHub.

    For very large deployments this should page teams with continue-as-new to
    keep dispatcher history bounded; today the active-team count is small enough
    to fan out in one pass (capped + logged in list_active_code_teams)."""
    ...
    async def run(self, input: EvaluateCodeWorkstreamsInput) -> None:
        ...
        # return_exceptions: one team's failure must not abort the whole cycle.
